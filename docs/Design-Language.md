...
**Edit-in-place popover (owner request, 2026-08-27).** The N11 split above is refined, not replaced — right-click still only adds and left-click still selects — but both now route through **one edit-in-place popover** shared by all four item types (phase, subtask, checkpoint, task) on both the Gantt and the calendar:
...
- **Right-click → New subtask/checkpoint/task** creates the item immediately (undo toast intact) and opens the popover on it with the name selected — "create, then edit". The old inline name field is retired: it was overwritten by the next menu press, and the popover is where the name is typed.
